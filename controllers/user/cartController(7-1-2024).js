...
const updateCart = async (req, res) => {
  try {
    ...
    // Fetch the current stock for the product associated with the cart item
    ...
    // Check if the quantity requested exceeds the max allowed (4)
    if (newValue > 4) {
      return res.json({
        success: false,
        message: "You can only choose up to 4 units of this product.",
    ...
    // Check if newValue is less than 1
    if (newValue < 1) {
      return res.json({
        success: false,
    ...
    if (newValue > currentStock) {
      return res.json({
        success: false,
        message: `Only ${currentStock} units are available in stock.`,
      });
    }

    // Calculate the new total value for the cart item based on the new quantity
    const price = oldCart.price; // Assuming price is stored in oldCart
    const updatedValue = newValue * price;
    ...
    // Calculate subtotal for user's cart
    const subTotal = await Cart.aggregate([
      { $match: { userId: ID } },
      { $group: { _id: null, total: { $sum: "$value" } } },
      { $project: { _id: 0, total: 1 } }
    ...
    // Prepare updated cart data to return to user
    const newData = updatedCart.map((data) => {
      const newDataItem = { ...data };

      // Set totalAmount based on current stock
      if (currentStock <= 0) {
        newDataItem.totalAmount = "Out of Stock"; // Set totalAmount to a string if out of stock
        newDataItem.outOfStock = true; // Mark product as out of stock
      } else {
        newDataItem.totalAmount = updatedValue;
      }
    ...
    // Send response with updated cart data and subtotal
    res.json({
      success: true,
      message: "Cart updated successfully",
      cartProd: newData,
    ...
    try {
      ...
      if (invalidQuantities.length > 0) {
        const quantityMessages = invalidQuantities.map(item => {
          return `${item.productName} available only ${item.availableStock} in stock`;
        });
